...
You can now design evaluations (Step 1), build datasets (Step 2), define scoring rubrics
(Step 3), structure eval pipelines (Step 4), deploy eval infrastructure (Step 5), run
adversarial tests (Step 6), and conduct safety-critical red teaming (Step 7). Each of
these skills produces data. None of them produces a decision.

The gap between an eval score and a deployment decision is where most evaluation misuse
lives. An eval that scores a model at 85% accuracy is data. Whether 85% is good enough
is a judgment. Whether the 85% is stable or a noisy estimate of a number that might
really be 78% is a statistical question. Whether the 85% means anything for your
specific use case is a validity question. And whether the person reading your eval report
understands all of this determines whether the eval leads to a good decision or a bad
one.
...
- **Screening evals** (is this model worth investigating further?): 50-100 samples.
  You accept wide CIs because the cost of a false positive is just wasted investigation
  time.

- **Comparison evals** (is model A better than model B?): 200-500 samples.
  You need enough precision to detect meaningful differences (see Section 3).

- **Deployment evals** (is this model safe/accurate enough for production?): 500-2000
  samples. The CI needs to be narrow enough that even the worst-case bound meets your
  sufficiency threshold.

- **Safety-critical evals** (could this model cause serious harm?): as many as you
  can get, plus qualitative analysis. No sample size makes a safety claim fully
  reliable, because the dangerous cases are by definition rare in the population.
...
The eval dataset is a sample from some population. The model will be deployed to serve
a different population. The gap between these populations determines how much the eval
score transfers to production performance.

Common mismatches:

| Eval Population | Production Population | Risk |
|----------------|----------------------|------|
| English only | Multilingual users | Performance on non-English inputs is unknown |
| Clean, well-formed queries | Noisy, abbreviated, ambiguous queries | Model may fail on real user input patterns |
| Curated by ML engineers | Generated by domain experts or end users | Eval may not cover domain-specific edge cases |
| Academic benchmark tasks | Business-specific tasks | Academic performance may not predict business performance |
| Current data (2024) | Future data (2025+) | Temporal shift degrades predictions |

The honest approach: state explicitly which population your eval samples from and which
population the model will serve. If they differ, state that the eval score is a lower
bound on uncertainty, not an estimate of production performance.
...
Three models are evaluated on five benchmarks using 500 paired samples per benchmark.
The results are:
...
- All models were evaluated on the exact same 500 test cases per benchmark
- Discordant pair counts (cases where models disagreed) average 8% of total
  samples per comparison

**Deliverable:** A one-page analysis that answers:
1. How many pairwise comparisons are you making? (Count models x benchmarks)
2. What is the Bonferroni-corrected significance threshold?
3. For each comparison, is the difference likely significant given the sample size
   and typical discordant pair count?
4. Which comparisons (if any) would you report as meaningful?
5. How would your conclusions change if you only had 100 samples per benchmark?

**Evaluation criteria:**
- Correct application of Bonferroni correction
- Correct reasoning about statistical power given sample sizes
- Honest conclusion about which differences are reportable
- Recognition that most of these differences are probably noise
...
- **"Challenges in Evaluating AI Systems"** - Ganguli et al., Anthropic (October
  ...
  reference for this step. Covers prompt format sensitivity, benchmark limitations,
  the BBQ example, and honest assessment of what makes evaluation hard. Read the
  full paper, not just the summary.

- **"An Introduction to the Bootstrap"** - Efron & Tibshirani (1993). Chapman &
  Hall/CRC. The definitive reference on bootstrap methods. Chapters 1-6 cover the
  core ideas used in this step. You do not need to read the entire book, but
  understanding the resampling intuition will serve you in any quantitative work.

- **METR Evaluation Reports** - evaluations.metr.org. Published evaluation reports
  for frontier models. Study the structure: methodology, findings, limitations. These
  reports are the de facto standard for honest safety eval reporting.
...
- **"Alignment Faking in Large Language Models"** - Greenblatt et al. (December
  ...
  limitation reporting. The authors explicitly state what their results do not show,
  preventing overinterpretation.

- **Anthropic Responsible Scaling Policy** - anthropic.com/rsp. Read for the
  framework architecture: capability thresholds, required safeguards, ASL levels.
  The RSP is an example of eval results driving deployment decisions at the
  organisational level, not just the technical level.
...
Individual eval skill is necessary but not sufficient. An engineer who writes excellent
eval reports matters only if someone reads them and acts on the findings. A deployment
gate matters only if it has the authority to block deployment. An eval suite matters
only if someone maintains it as the domain evolves. Step 9 covers eval ownership, eval
review as a team practice, continuous evaluation, and the relationship between eval
results and governance decisions.

The connection to this step is direct: the interpretation and communication skills from
Step 8 are the foundation of eval culture. A team that reports results honestly, includes
